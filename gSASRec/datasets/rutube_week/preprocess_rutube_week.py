...
FILE_NAME = DATASET_DIR / "rutube_week_train.txt"
FILE_NAME_TEST = DATASET_DIR / "rutube_week_test.txt"
...
def train_val_test_split():
    ...
    # user_items = {user: items for user, items in user_items.items()}  #  if len(items) >= 4}
    ...
    with open(FILE_NAME_TEST) as f:
        for line in f:
            ...
            user = int(user) + 1
            item = int(item)
            test_user_items[user] = items_renumbering[item]
    ...
    for user in user_items:
        if user in val_users:
            ...
            val_input_sequence = user_items[user][:-1]
            val_gt_action = user_items[user][-1]
            val_input_sequences.append(val_input_sequence)
            val_gt_actions.append(val_gt_action)
        else:
            train_input_sequence = user_items[user]
            train_sequences.append(train_input_sequence)

        if user in test_user_items:
            test_input_sequence = user_items[user]
            test_gt_action = test_user_items[user]
