...
class PopTargetItemsSampler(TargetItemSampler):
    def get_sampled_ranking_requests(self):
        # Список айтемов и соответствующая им частотная вероятность
        items, probs = SampledProxy.all_item_ids_probs(self.actions)
        # Dict: user_id to his actions
        ...
        for user_id in by_user_test:
            ...
            while len(target_items) < self.target_size:
                # Если в test у user_id меньше, чем target_size айтемов, то сэмплируем из общей выборки случайными
                item_ids = np.random.choice(
                    items,
                    self.target_size - len(target_items),
                    p=probs,
                    replace=False
                )
